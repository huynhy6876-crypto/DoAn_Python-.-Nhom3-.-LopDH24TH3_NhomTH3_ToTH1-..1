...
# ====================== CRUD CHUNG – HOÀN CHỈNH + TÌM KIẾM ======================
def open_crud_window(prev, table, cols, labels):
    ...
    def them():
        ...
        if table == "NhanVien":
            ...
            insert_cols = ["ma_nv", "ten_nv", "gioi_tinh", "dia_chi", "sdt", "chuc_vu"]
            ...
            insert_cols = ["ma_kh", "ten_kh", "dia_chi", "sdt"]
    ...
    def chon_dong(event):
        ...
        for i, val in enumerate(values):
            ...
            if label in ["Mã NV", "Mã SP", "Mã KH"]:
                ...
                try:
                    entries[label].set(val)
                except:
                    pass
            else:
                entries[label].delete(0, tk.END)
                entries[label].insert(0, str(val) if val is not None else "")
...
def open_sales_window(prev):
    ...
    def pay():
        ...
        if conn:
            cur = conn.cursor()
            try:
                cur.execute("INSERT INTO HoaDon (ma_hd, ma_kh, ma_nv, ngay_lap, tong_tien) VALUES (%s, %s, %s, %s, %s)",
                            (ma_hd, ma_kh, nv, datetime.now().date(), total))
                for item in cart:
                    cur.execute("INSERT INTO ChiTietHoaDon (ma_hd, ma_sp, so_luong, don_gia) VALUES (%s, %s, %s, %s)",
                                (ma_hd, item["ma_sp"], item["sl"], item["gia"]))
                    cur.execute("UPDATE SanPham SET so_luong_ton = so_luong_ton - %s WHERE ma_sp = %s", (item["sl"], item["ma_sp"]))
                conn.commit()
                messagebox.showinfo("THÀNH CÔNG", f"Thanh toán {total:,.0f} VND\\nMã HD: {ma_hd}")
                reset()
            except mysql.connector.Error as e:
                messagebox.showerror("Lỗi CSDL", f"Thanh toán thất bại: {e}")
            finally:
                cur.close()
                conn.close()
    ...
    tk.Button(btn_f, text="THANH TOÁN", font=("Time New Roman", 14, "bold"), bg="#27ae60", fg="white", width=22, height=2, command=pay).pack(side="right", padx=40)
